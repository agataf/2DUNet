...
class Model(nn.Module):
  ...
  def __init__(self, D_in, output_spec, depth, io_size=io_size, io_stride=io_stride, bn=bn):
    ...
    # Downsample input
    self.add_downsample_mod(scale_factor=(1,4,4), mode='bilinear')     
    ...
    self.add_upsample_mod(scale_factor=(1,4,4), mode='bilinear')

    #m = nn.Upsample(scale_factor=(1,2,2), mode='bilinear')
    #self.outputdeconv = m(self.outputdeconv)
    # TODO: insert upsampling here      
         
  def add_downsample_mod(self, scale_factor=(1,4,4), mode='bilinear'):
    setattr(self, "downsample",
            nn.AvgPool3d(scale_factor))  

  def add_upsample_mod(self, scale_factor=(1,4,4), mode='bilinear'):
